...
class Seasons(Enum):
    ...
    VALIDATION = "ROIs0000_validation"
...
if __name__ == "__main__":
    ...
    # Load all bands of all patches in the validation set
    print('Loading validation data...')
    start = time.time()
    s1, s2, lc, _ = sen12ms.get_triplets(Seasons.VALIDATION, 0, s1_bands=S1Bands.ALL, 
                                        s2_bands=S2Bands.ALL, lc_bands=LCBands.igbp)
    print(f"Validation: S1: {s1.shape}, S2: {s2.shape}, LC: {lc.shape}")
    print("Time Taken {}s".format(time.time() - start))
