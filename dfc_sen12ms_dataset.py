...
class DFCSEN12MSDataset:
    ...
    def get_s1_s2_lc_dfc_quad(self, season, scene_id, patch_id, s1_bands=S1Bands.ALL, s2_bands=S2Bands.ALL, lc_bands=LCBands.ALL, dfc_bands=LCBands.NONE, include_dfc=False):
        """
...
if __name__ == "__main__":
    ...
    patch_cnt = 0
    for s in Seasons.VALIDATION.value:
    ...
    validation_ids = sen12ms.get_season_ids(Seasons.FALL)
    for i, (scene_id, patch_ids) in enumerate(validation_ids.items()):
        if i >= 3:
            break

        s1, s2, lc, dfc, bounds = sen12ms.get_s1_s2_lc_dfc_quad(Seasons.FALL, scene_id, patch_ids[0], s1_bands=S1Bands.ALL, 
                                                            s2_bands=[S2Bands.red, S2Bands.nir1], lc_bands=LCBands.LC, dfc_bands=LCBands.DFC, include_dfc=True)

        print(f"Scene: {scene_id}, S1: {s1.shape}, S2: {s2.shape}, LC: {lc.shape}, DF: {dfc.shape}, Bounds: {bounds}")
    ...
    print(f"Scene: 106, S1: {s1.shape}, S2: {s2.shape}, LC: {lc.shape}, DFC: {dfc.shape}")
